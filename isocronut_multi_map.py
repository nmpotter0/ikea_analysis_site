...
def build_url(origin='',
    ...
    prefix = 'https://maps.googleapis.com/maps/api/distancematrix/json?mode=driving&units=imperial&avoid=ferries'
    if access_type == 'personal':
        url = urllib.parse.urlparse('{0}&origins={1}&destinations={2}&key={3}'.format(prefix,
                                                                                  origin_str,
                                                                                  destination_str,
                                                                                  key))
        ...
        return full_url
    if access_type == 'business':
        url = urllib.parse.urlparse('{0}&origins={1}&destinations={2}&departure_time={3}&client={4}'.format(prefix,
                                                                                                        origin_str,
                                                                                                        destination_str,
...
def geocode_address(address='',
    ...
    if access_type == 'personal':
        url = urllib.parse.urlparse('{0}?address={1}&key={2}'.format(prefix,
                                                                 address_str,
                                                                 key))
        full_url = url.scheme + '://' + url.netloc + url.path + '?' + url.query
    if access_type == 'business':
        url = urllib.parse.urlparse('{0}?address={1}&client={2}'.format(prefix,
                                                                    address_str,
                                                                    client))
        # Get the private_key used to sign the API request
        private_key = config.get('api', 'crypto_key')
    ...
    ascii_url = removeNonAscii(full_url)
    logging.debug(ascii_url)

    # Request geocode from address
    req = urllib.request.Request(ascii_url)
    opener = urllib.request.build_opener()
    f = opener.open(req)
    d = simplejson.load(f)
...
def generate_isochrone_map(origin='',
    ...
    iso_out = []
    for i in iso:
        iso_out.append([i[1],i[0]])

    geojsonWrite = '{ "type": "FeatureCollection", "features": [{"geometry": {"type": "Polygon", "coordinates": ['
    geojsonWrite += str(iso_out)
    geojsonWrite += ']}, "properties": {"Store": "Ikea", "Location": "Address"}}]}' 

    with open('isochrone.html', 'w') as f:
        f.write(htmltext)

    with open('isoSW2.geojson', 'w') as f:
        f.write(str(geojsonWrite))

    return iso

def location_scrape():
    """
    Scrape multiple locations from a ikea site
    """
    res = requests.get('http://info.ikea-usa.com/StoreLocator/StoreLocator.aspx')

    siteSoup = bs4.BeautifulSoup(res.text, "html.parser")
    elem =  siteSoup.select('script')
    cdataPull = elem[32].getText()
    storeDataRegex = re.compile(r'(name = \')(.{,25})(\';stores)(.{,800})(latitude = )(\d\d\.\d+)(;stores\[stores\.length-1\]\.longitude = )(-?\d+.\d+)(;stores)')
    coord_search = storeDataRegex.findall(cdataPull)
    ikeaCoords= []
    for i in coord_search:
        ikeaCoords.append((i[1],[float(i[5]),float(i[7])]))

    print(ikeaCoords)

    return ikeaCoords

def get_mult_isochrone(origin=location_scrape(),
                            duration=120,
                           number_of_angles=40,
                           tolerance=0.5,
                           access_type='personal',
                           config_path='config/'):
    """
    Call the get_isochrone function for multiple coordinates and generate geojson & web map using output.
    """
    ...
    geojsonWrite = '{ "type": "FeatureCollection", "features": ['

    count = 1
    for i in range(origin_length):
        
        dataPoint = origin[count-1]

        this_coord = dataPoint[1]
        this_name = str(dataPoint[0])

        htmltext += """var marker = new google.maps.Marker({{
          position: new google.maps.LatLng({0},{1}),
          map: map,
          title: {2}
        }}); var isochrone;""".format(this_coord[0], this_coord[1], this_name)

        result = get_isochrone(this_coord, duration, number_of_angles, tolerance, access_type, config_path)

        htmltext += """var isochroneCoords = [
        """.format(this_coord[0], this_coord[1])
        ...
        #writing geojson for qgis use
        geojsonWrite += '{"geometry": {"type": "Polygon", "coordinates": ['
        geojsonWrite += str(iso_out)
        geojsonWrite += ']}, "properties": {"Name": "'
        geojsonWrite += this_name
        geojsonWrite += '"}}'
        if count < origin_length:
            geojsonWrite += ', '
        else:
            geojsonWrite += ']}'
        count += 1
    ...
    with open('multipoly.html', 'w') as f:
        f.write(htmltext)

    with open('multipoly.geojson', 'w') as f:
        f.write(str(geojsonWrite))
